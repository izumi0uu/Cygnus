...
from cygnus.backend.services.auth_service import get_current_user
from cygnus.backend.services.permission_engine import (
    _get_user_permissions,
    has_any_permission,
)
from cygnus.backend.routers.wiki_drafts import DraftResponse, _draft_response
...
async def submit_branch(
    ...
    from cygnus.backend.services.contribution_service import notify_submitted, wiki_draft_adapter
    for d in drafts:
        if d.status != "pending":
            d.status = "pending"
...
async def close_branch(
    ...
    from cygnus.backend.services.contribution_service import withdraw, wiki_draft_adapter
    for d in drafts:
    ...
    try:
        async with db.begin_nested():
            for d in drafts:
                ...
                from cygnus.backend.services.contribution_service import notify_approved, wiki_draft_adapter
...
async def rebase_draft(
    ...
    from cygnus.backend.services.contribution_service import _enqueue_ai_review
