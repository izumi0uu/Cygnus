...
async def ingest_source(
    ...
    """
    Ingest a Source into governed draft staging:
      ...
      5. Compile into durable wiki drafts; review/approval/publish happen later
    """
    ...
    try:
        ...
        if not pages_data or not any(
            (page.get("content") or "").strip() for page in pages_data
        ):
            source.status = "error"
                ...
                for idx, image in enumerate(images, 1):
                    try:
                        if idx % 5 == 0 or idx == 1 or idx == len(images):
                            logger.info(
                                f"Vision AI analyzing image {idx}/{len(images)}..."
                            )
                        ...
                        logger.warning(
                            f"Failed to analyze image {image.minio_key}: {exc}"
                        )
        ...
        # Draft staging is all-or-nothing. If any generated operation cannot
        # stage, roll back every nested draft write before source error state is
        # persisted; a partial compiler result must never mark this source ready.
        ...
        await session.flush()
        logger.success(
            f"Source {source_id} compiled into drafts: "
            f"+{result['drafts_created']} create drafts, "
            f"+{result['edit_drafts_created']} edit drafts"
        )
        return source
