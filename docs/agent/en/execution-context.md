...
- Runtime MCP and session capabilities share one adapter-definition contract for four R0 governed retrieval tools, R0 `list_drift_alerts`, the R1 `propose_knowledge_object` / `update_draft_object` / `request_review` lifecycle adapters, R0 `read_review_feedback`, and durable-core `validate_publish_policy` / `publish_knowledge_object`. `list_drift_alerts` reloads scoped durable release/incident signals, resolves active visible audience bindings in batch, and returns `ready`/`partial`/explicit-coverage `unavailable` without hiding provider failures; only `record_feedback_signal` remains explicitly `not_exposed`. The R1 tools are contributor-gated in the catalog, while drift alerts and feedback reads are authenticated-gated; every state-changing adapter re-checks scoped identity, author authority, source visibility, and optimistic draft version server-side.
