...
- 新 feedback 调用在同一个 caller-owned transaction 内暂存原始 `GovernanceFeedbackSignal`、适用时的 mapped `GovernanceFeedbackRoute` 与恰好一条 runtime mutation `AuditLog`；persistence owner 只 flush，exact replay 不产生重复 audit。feedback 以 `RESTRICT` 外键保留 `actor_id`、`page_id`、`draft_id` 链接；`object_id` 与 `source_context_ref` 是持久化上下文，source context 不是 `Source` 外键。提供 `object_id` 时，它必须是 governed retrieval 返回的规范、不可变 `ko-page-<WikiPage UUID>`：resolution 解析 UUID，将 actor 的 SQL read scope 应用于 `WikiPage.id`，并且绝不回退到可变 slug。draft resolution 同样在 SQL scope 内执行；隐藏或缺失的 object/draft ref，以及非规范的 object ref，统一返回 `not_found`；冲突 ref 返回不泄露资源的 `conflict` 且不写入记录。每个 state-changing adapter 都必须在服务端重新检查 identity、作用域和资源权限。
...
- CYG-122 的 `resolved-ticket-export/v1` 只接收已脱敏 CSV/JSONL；`source_ref` 必须代表不可变 snapshot。Importer 必须先完成全包验证，再确定性形成 audience-aware cluster；未达阈值候选不得写 review truth，达阈值候选只能通过既有 `GovernanceSignal` + review assignment owner 持久化结构化 evidence refs。Exact replay 保持同一 signal；同一 `source_ref` 的事实漂移必须 conflict；任何成功导入都不等于 draft 已创建、review 已完成或知识已发布。
- CYG-125 要求每次 resolved-ticket import 显式绑定一个已存在且为 `ready` 的 `Source` UUID；`source_ref` 仍是不变 export identity，`source_id` 是 durable Source Connector/evidence linkage。达阈值 signal 必须持久化该 ID，promotion/approval 必须把它原样传入 `WikiPage.source_ids`；相同绑定可 exact replay，同一 export truth 更换 Source 必须 conflict。不得自动创建 Source，也不得绕过 durable publish 的 ready-evidence 与 active audience-binding gate。
