...
from cygnus.backend.ai.providers.base import (
    EmbeddingProvider,
    LLMProvider,
    ProviderConfig,
    ProviderType,
...
DEFAULT_CUSTOM_LLM_CONTEXT_WINDOW_TOKENS = 256_000
DEFAULT_CUSTOM_LLM_MAX_OUTPUT_TOKENS = 16_384
CUSTOM_LLM_REASONING_EFFORTS = {
    "none",
    "minimal",
    "low",
    "medium",
...
def _parse_reasoning_effort(value: Optional[str]) -> Optional[str]:
    ...
    if not normalized:
        return None
    if normalized in CUSTOM_LLM_REASONING_EFFORTS:
        return normalized
    return None


@dataclass(frozen=True)
class CustomLLMSettings:
    enabled: bool
    provider: str
    model_id: str
    base_url: str
    ...
    def build_spec(self):
        from cygnus.backend.ai.llm_catalog import LLMModelSpec
        ...
        return LLMModelSpec(
            id=f"custom:{self.provider}/{self.model_id}",
            ...
            label=f"Custom {provider_label} ({self.model_id})",
...
def _get_embedding_class(provider: ProviderType) -> type[EmbeddingProvider]:
    if provider == ProviderType.GOOGLE:
        from cygnus.backend.ai.providers.google import GoogleEmbedding
        return GoogleEmbedding
    elif provider == ProviderType.OPENAI:
        from cygnus.backend.ai.providers.openai_provider import OpenAIEmbedding
...
def _get_llm_class(provider: ProviderType) -> type[LLMProvider]:
    if provider == ProviderType.GOOGLE:
        from cygnus.backend.ai.providers.google import GoogleLLM
        return GoogleLLM
    elif provider == ProviderType.OPENAI:
        from cygnus.backend.ai.providers.openai_provider import OpenAILLM
        return OpenAILLM
    elif provider == ProviderType.ANTHROPIC:
        from cygnus.backend.ai.providers.anthropic_provider import AnthropicLLM
        return AnthropicLLM
...
def _get_vision_class(provider: ProviderType) -> type[VisionProvider]:
    if provider == ProviderType.GOOGLE:
        from cygnus.backend.ai.providers.google import GoogleVision
        return GoogleVision
    elif provider == ProviderType.OPENAI:
        from cygnus.backend.ai.providers.openai_provider import OpenAIVision
        return OpenAIVision
...
class ProviderRegistry:
    ...
    async def get_active_embedding_spec_id(self) -> Optional[str]:
        ...
        from cygnus.backend.ai.embedding_catalog import EMBEDDING_CATALOG
    ...
    async def get_active_llm_spec_id(self) -> Optional[str]:
        ...
        from cygnus.backend.ai.llm_catalog import LLM_CATALOG, derive_spec_id
        from cygnus.backend.services.config_service import ACTIVE_LLM_MODEL_KEY, ConfigService
    ...
    async def get_custom_llm_settings(self) -> CustomLLMSettings:
        """Return the Hermes-style custom LLM relay configuration."""
        from cygnus.backend.services.config_service import (
            LLM_CUSTOM_API_KEY,
        ...
        reasoning_effort = _parse_reasoning_effort(
            await svc.get(LLM_CUSTOM_REASONING_EFFORT_KEY)
        )
        if provider != ProviderType.OPENAI.value:
            reasoning_effort = None

        return CustomLLMSettings(
            enabled=_parse_bool(await svc.get(LLM_CUSTOM_ENABLED_KEY)),
            provider=provider,
            model_id=(await svc.get(LLM_CUSTOM_MODEL_ID_KEY) or "").strip(),
            base_url=(await svc.get(LLM_CUSTOM_BASE_URL_KEY) or "").strip(),
            api_key=(await svc.get(LLM_CUSTOM_API_KEY) or "").strip(),
            context_window_tokens=_parse_positive_int(
                await svc.get(LLM_CUSTOM_CONTEXT_WINDOW_KEY),
                DEFAULT_CUSTOM_LLM_CONTEXT_WINDOW_TOKENS,
            ),
            max_output_tokens=_parse_positive_int(
                await svc.get(LLM_CUSTOM_MAX_OUTPUT_KEY),
    ...
    async def get_active_vision_spec_id(self) -> Optional[str]:
        ...
        from cygnus.backend.ai.vision_catalog import VISION_CATALOG, derive_spec_id
        from cygnus.backend.services.config_service import (
    ...
    async def _load_embedding_config(
        ...
        """
        from cygnus.backend.ai.embedding_catalog import get_spec
        from cygnus.backend.services.config_service import (
            ACTIVE_EMBEDDING_MODEL_KEY,
            ConfigService,
    ...
    async def _load_llm_config(self) -> ProviderConfig:
        ...
        """
        from cygnus.backend.ai.llm_catalog import get_spec
        from cygnus.backend.services.config_service import ConfigService

        svc = ConfigService(self.db)
        custom = await self.get_custom_llm_settings()
        if custom.enabled:
            if not custom.model_id or not custom.base_url:
                raise ValueError(
                    "Custom LLM relay is enabled but incomplete. Fill model ID and base URL."
                )

            spec = custom.build_spec()
            return ProviderConfig(
                provider=ProviderType(custom.provider),
    ...
    async def _load_vision_config(self) -> ProviderConfig:
        ...
        from cygnus.backend.ai.vision_catalog import get_spec
        from cygnus.backend.services.config_service import ConfigService
...
def supported_providers() -> dict:
    ...
    """
    from cygnus.backend.ai.embedding_catalog import EMBEDDING_CATALOG
    from cygnus.backend.ai.llm_catalog import LLM_CATALOG
    from cygnus.backend.ai.vision_catalog import VISION_CATALOG
