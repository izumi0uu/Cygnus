...
from cygnus.backend.database.models import Employee, ScopeType, Source, SourceDepartment, WikiPage
from cygnus.backend.database.repository import Repository
from cygnus.backend.services.audit_service import log_audit
from cygnus.backend.services.auth_service import (
    get_current_user,
    require_permission,
)
...
async def _image_count(session: AsyncSession, source_id: uuid.UUID) -> int:
    ...
    from cygnus.backend.database.models import SourceImage
...
async def get_source(
    ...
    from cygnus.backend.services.permission_engine import can_access_document
...
async def update_source(
    ...
    if body.department_ids is not None:
        ...
        if old_dept_ids != new_dept_ids and source.status == "ready" and not source.preserve_verbatim:
            ...
            from cygnus.backend.ai.mrp.pipeline import _resolve_wiki_scopes
...
async def approve_extraction(
    ...
    """
    from cygnus.backend.worker import enqueue_post_extraction_pipeline
