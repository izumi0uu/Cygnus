...
- report 只提供确定性回归证据。CYG-120 已在该 gate 之外实现权限内 feedback-route 运维读取与结构化 worker-outcome event；但 CYG-117 report 本身仍不执行或证明 feedback-route worker 执行、在线 business KPI instrumentation 或 business-impact evidence。
...
CYG-117 既不执行 feedback routing，也不测量这些 KPI。CYG-120 的运维读取与结构化事件可以证明 route backlog、lease/retry/terminal outcome 与 linked review truth；它们仍不能把 domain report 通过或 route completion 表述成 business impact。
...
3. 以 CYG-120 的权限内 route operations 与 worker outcome event 作为独立在线运行证据，再单独建设 support KPI instrumentation；不得把两层合并
...
generic agent benchmark、judge-model quality score、durable feedback routing、feedback-route worker 执行与在线 business impact 均不属于 CYG-117 gate，也不能由该 report 证明；CYG-120 的运维读面和 worker event 位于 gate 之外，只证明运行生命周期真相，不证明业务影响。
