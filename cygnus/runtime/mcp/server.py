...
def create_mcp_server() -> FastMCP:
    ...
    mcp = FastMCP(
        ...
        instructions=(
            ...
            "5. `propose_knowledge_object` — create a durable staged draft; it is not "
            "reviewed or published truth.\n"
            "6. `update_draft_object` — make a version-checked draft revision.\n"
            "7. `request_review` — submit the current draft version through Cygnus review.\n"
            "8. `read_review_feedback` — read scoped review and approval feedback.\n"
            "9. `validate_publish_policy` — re-check approval, source readiness, audience bindings, "
            "and object version before a durable command.\n"
            "10. `publish_knowledge_object` — administrator-only durable publication; a successful "
