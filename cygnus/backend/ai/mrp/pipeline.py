...
from cygnus.backend.ai.mrp.mapper import run_map_phase
from cygnus.backend.ai.mrp.reducer import run_reduce_phase
from cygnus.backend.ai.mrp.verifier import run_verify_phase
from cygnus.backend.ai.mrp.writer import PageWriteResult, run_refine_phase
...
async def _resolve_wiki_scopes(session: AsyncSession, source) -> list[tuple[str, Optional[uuid.UUID]]]:
    ...
    """
    from cygnus.backend.database.models import Source as SourceModel
    from cygnus.backend.database.models import SourceDepartment
...
async def run_commit_phase(
    ...
    """
    from cygnus.backend.ai.mrp.merger import merge_page_content
...
async def _load_plan(session: AsyncSession, source_id: uuid.UUID):
    ...
    from cygnus.backend.database.models import SourceCompilationPlan
...
async def _load_chunk_extracts(session: AsyncSession, source_id: uuid.UUID) -> list:
    ...
    from cygnus.backend.database.models import SourceChunkExtract
    rows = (await session.execute(
...
async def _auto_trigger_refine(source_id: uuid.UUID, plan) -> dict:
    ...
    from cygnus.backend.worker import get_arq_pool
