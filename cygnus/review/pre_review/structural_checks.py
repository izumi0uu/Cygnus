...
from cygnus.runtime.database.models import WikiPage
...
async def run(
    ...
    if targets:
        ...
        # is broken.
        rows = (
            await db.execute(select(WikiPage.slug).where(WikiPage.slug.in_(targets)))
        ).all()
    ...
    out.append(
        {
            ...
            "message": (
                f"Content is unusually short ({length} chars)"
                if too_short
