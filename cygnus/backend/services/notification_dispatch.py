...
Configuration keys (read via ConfigService):
  ...
  smtp_from              -> string e.g. "Cygnus <noreply@cygnus.example>"
  smtp_use_tls           -> "true" / "false" (default "true")

  webhook_enabled        -> "true" / "false"
  webhook_url            -> string (single endpoint; multi-URL is a future ext)
  webhook_secret         -> optional HMAC secret (sent in X-Cygnus-Signature)
...
from cygnus.backend.database.models import Employee, Notification
...
async def _dispatch_webhook_batch(
    ...
    if secret:
        ...
        headers["X-Cygnus-Signature"] = f"sha256={sig}"
