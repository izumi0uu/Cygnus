...
def test_pyproject_carries_imported_backend_dependency_surface() -> None:
    ...
    for prefix in expected_prefixes:
        assert any(dep.startswith(prefix) for dep in deps), (
            f"missing dependency wiring for {prefix}"
        )
...
def test_seed_skills_wiring_module_exists() -> None:
    ...
    assert script.is_file(), (
        "expected cygnus/runtime/bootstrap/seed_builtin_skills.py to exist for app.main startup wiring"
    )
...
def test_main_references_existing_seed_skills_module() -> None:
    main_tree = ast.parse(Path("cygnus/runtime/main.py").read_text(encoding="utf-8"))
    imports_seed_builtin_skills = any(
        isinstance(node, ast.ImportFrom)
        and node.module == "cygnus.runtime.bootstrap.seed_builtin_skills"
        and any(alias.name == "seed_builtin_skills" for alias in node.names)
        for node in ast.walk(main_tree)
    )

    assert imports_seed_builtin_skills
    ...
    for path in root.rglob("*.py"):
        ...
        for node in ast.walk(tree):
            if (
                isinstance(node, ast.ImportFrom)
                and node.module
                and node.module.startswith("cygnus.runtime.")
            ):
                ...
                for alias in node.names:
                    if (
                        alias.name.startswith("cygnus.runtime.")
                        and alias.name not in local_modules
                    ):
