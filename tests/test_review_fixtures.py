...
class ReviewFixtureTests(unittest.TestCase):
    def test_sample_bundles_cover_first_review_home_scenarios(self) -> None:
        ...
        self.assertEqual(
            risk_types,
            {"source_blindness", "audience_mismatch", "drift", "ticket_pressure"},
        )
    ...
    def test_sample_command_brief_matches_cyg6_shape(self) -> None:
        ...
        self.assertGreaterEqual(len(priority_items), 4)
        self.assertIn("headline", payload)
        self.assertIn("summary_counts", payload)
        self.assertIn("why_now", priority_items[0])
        self.assertEqual(summary_counts["ticket_pressure"], 1)
        self.assertEqual(summary_counts["audience_mismatch"], 1)
