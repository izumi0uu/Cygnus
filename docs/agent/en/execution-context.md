...
- `list_drift_alerts` reloads scoped durable signal truth in the request DB session. Authenticated R1 `record_feedback_signal` accepts only the six fixed feedback types and stages a dedicated `GovernanceFeedbackSignal` plus runtime mutation `AuditLog` in the caller transaction. The runtime audit is not a `GovernanceLedgerEvent`; Runtime MCP commits both feedback records only after successful persisted output.
- Feedback preserves `actor_id`, `page_id`, and `draft_id` as `RESTRICT` foreign-key links; `object_id` and `source_context_ref` are stored context, and the source context is not a `Source` foreign key. Object/draft resolution is SQL-scoped; hidden, absent, or ambiguous refs share `not_found`, while conflicting refs return non-disclosing `conflict` without writes. Success remains `recorded_only` with both queue flags false. Every state-changing adapter re-checks identity, scope, and resource authority server-side.
