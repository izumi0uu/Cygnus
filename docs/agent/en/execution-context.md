...
- `list_drift_alerts` reloads scoped durable signal truth in the request DB session. Authenticated R1 `record_feedback_signal` accepts only the six fixed feedback types and requires a nonblank `command_id` of at most 220 characters. `GovernanceFeedbackSignal.command_id` is globally unique and has a 64-character `request_fingerprint` over normalized payload and authenticated actor; exact replay returns the existing signal/route without writes, reuse with a changed normalized payload or actor returns structured `conflict` without writes, and a new command ID remains distinct feedback.
- A new feedback call stages the original `GovernanceFeedbackSignal`, its mapped `GovernanceFeedbackRoute` when applicable, and exactly one runtime mutation `AuditLog` in the same caller-owned transaction; the persistence owner only flushes, and exact replay creates no duplicate audit. Feedback preserves `actor_id`, `page_id`, and `draft_id` as `RESTRICT` foreign-key links; `object_id` and `source_context_ref` are stored context, and the source context is not a `Source` foreign key. Object/draft resolution is SQL-scoped; hidden, absent, or ambiguous refs share `not_found`, while conflicting refs return non-disclosing `conflict` without writes. Every state-changing adapter re-checks identity, scope, and resource authority server-side.
- `GovernanceFeedbackRoute` is the only durable queue truth, unique on `(feedback_signal_id, route_kind)`, with `route_kind` `review|refresh` and `lifecycle_state:"queued"`. The mapping is frozen: `low_rating` → queued `review`, `stale_answer` → queued `refresh`, and all other accepted types → `recorded_only` with no route.
- Routed responses expose `route_id`, `route_ref=feedback-route:{uuid}`, `route_kind`, `route_state:"queued"`, and `routing_state:"review_queued"|"refresh_queued"`, with exactly one queue flag true. Non-routed responses expose null route fields, `routing_state:"recorded_only"`, and both queue flags false. No feedback worker or consumer is introduced; queued is durable intent only and never claims completed review or refresh work.
