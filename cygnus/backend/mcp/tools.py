"""
Cygnus MCP Tools — support knowledge + raw source drill-down for Claude.
...
from cygnus.backend.mcp.logging import current_identity, logged_tool
from cygnus.backend.mcp.permissions import (
    ANY_AUTHENTICATED,
    CAN_CONTRIBUTE_WIKI,
...
async def _get_identity():
    ...
    from cygnus.backend.services.mcp_auth_service import MCPAuthService
...
async def _get_allowed_source_ids(identity, session: Optional[AsyncSession] = None) -> Optional[set[str]]:
    ...
    from cygnus.backend.services.mcp_auth_service import apply_scope_filter
...
async def _can_review_page(session: AsyncSession, employee, page) -> bool:
    ...
    from cygnus.backend.services.permission_engine import (
...
async def _format_oos_hint(session: AsyncSession, oos_hits: list) -> str:
    ...
    from cygnus.backend.database.models import Department
...
def register_tools(mcp: FastMCP):
    ...
    async def search_wiki(query: str, top_k: int = 10) -> str:
        ...
        from cygnus.backend.database import async_session_factory
        from cygnus.backend.services import wiki_service
    ...
    async def read_wiki_page(slug: str) -> str:
        ...
        from cygnus.backend.database.models import Department, WikiPage
    ...
    async def get_source_pages(source_id: str, pages: str) -> str:
        ...
        from cygnus.backend.database import async_session_factory
        from cygnus.backend.database.models import Source
        from cygnus.backend.services.source_outline import parse_page_range, slice_pages_by_range
    ...
    async def propose_wiki_edit(
        ...
        from cygnus.backend.database import async_session_factory
        from cygnus.backend.database.models import Employee, WikiPage
        from cygnus.backend.services import wiki_service
        ...
        async with async_session_factory() as session:
            ...
            draft.page = page
            from cygnus.backend.services import contribution_service
            from cygnus.backend.services.contribution_service import wiki_draft_adapter
    ...
    async def list_pending_drafts(
        ...
        from cygnus.backend.services.permission_engine import _get_user_permissions
    ...
    async def review_draft(draft_id: str) -> str:
        ...
        from cygnus.backend.database import async_session_factory
        from cygnus.backend.database.models import Employee, WikiPageDraft
    ...
    async def request_changes_on_draft(draft_id: str, reviewer_note: str) -> str:
        ...
        from cygnus.backend.database.models import Employee, WikiPageDraft
        from cygnus.backend.services import contribution_service
        from cygnus.backend.services.contribution_service import (
            InvalidTransition,
            wiki_draft_adapter,
        )
    ...
    async def resubmit_draft(
        ...
        from cygnus.backend.database.models import Employee, WikiPageDraft
        from cygnus.backend.services import contribution_service
        from cygnus.backend.services.contribution_service import InvalidTransition
    ...
    async def propose_wiki_create(
        ...
        from cygnus.backend.services import contribution_service, wiki_service
        from cygnus.backend.services.contribution_service import wiki_draft_adapter
