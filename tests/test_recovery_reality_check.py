...
class RecoveryRealityCheckTests(unittest.TestCase):
    def test_build_downstream_reality_check_groups_feedback_by_command(self) -> None:
        ...
        strip = cast(dict[str, object], surface["reality_check_strip"])

        self.assertEqual(surface["surface_id"], "downstream-reality-check")
        self.assertEqual(strip["command_id"], "cmd-publish-1")
        self.assertTrue(strip["frontline_changed"])
        self.assertIn("copilot", cast(list[str], strip["converging_surfaces"]))
        self.assertIn("macro", cast(list[str], strip["lagging_surfaces"]))
        self.assertIn("queue_sidebar", cast(list[str], strip["lagging_surfaces"]))
        self.assertEqual(strip["unresolved_signal_count"], 3)
        self.assertEqual(
            cast(list[dict[str, object]], surface["feedback_feed"])[0]["signal_id"],
            "sig-accepted-1",
        )
        self.assertIn(
            "object:ko-invoice-export-enterprise-eu",
            cast(list[str], surface["upstream_object_links"]),
        )
        self.assertIn(
            "return_to_command_brief", cast(list[str], surface["send_back_commands"])
        )
    ...
    def test_reality_check_rolls_up_mismatch_by_audience(self) -> None:
        ...
        audience_map = {
            cast(str, item["audience_label"]): item
            for item in cast(list[dict[str, object]], surface["mismatch_by_audience"])
        }
        ...
        self.assertIn("macro", cast(list[str], enterprise_eu["affected_surfaces"]))
        ...
        self.assertIn("queue_sidebar", cast(list[str], free_us["affected_surfaces"]))
    ...
    def test_query_returns_surface_for_known_command(self) -> None:
        ...
        self.assertEqual(
            cast(dict[str, object], payload["reality_check_strip"])["command_type"],
            "publish",
        )
