...
class SkillService:
    ...
    async def _upsert_skill_db_records(
        ...
        all_depts = list(
            set(filter(None, (department_ids or []) + ([scope_id] if scope_id else [])))
        )
        ...
        if existing_skill:
            ...
            if scope_type == "department" or all_depts:
                ...
                existing_skill.scope_id = scope_id or (
                    all_depts[0] if all_depts else None
                )

                # Wipe and recreate Many-to-Many links
                await db.execute(
                    sa.delete(SkillDepartment).where(
                        SkillDepartment.skill_id == existing_skill.id
                    )
                )
                for d_id in all_depts:
                    db.add(
                        SkillDepartment(skill_id=existing_skill.id, department_id=d_id)
                    )
            else:
    ...
    async def upload_skills(
        ...
        results: list[dict[str, object] | Skill] = []
        duplicates = []
        jobs_to_enqueue = []
        ...
        try:
            for file in files:
                ...
                # A created/updated upsert always materialises a Skill; a None
                # here would be a contract violation we must not persist through.
                if skill is None or new_v_num is None:
                    results.append(
                        {
                            "name": name,
                            "status": "error",
                            "message": "Skill record could not be materialized.",
                        }
                    )
                    continue
                ...
                jobs_to_enqueue.append(
                    (str(skill.id), str(new_version.id), temp_path, file.filename)
                )
            ...
            for job_args in jobs_to_enqueue:
                await pool.enqueue_job(
                    "ingest_skill_task", *job_args, _queue_name="skills_queue"
                )
    ...
    async def reupload_skill(
        db: AsyncSession, slug: str, file: Any, current_user_id: uuid.UUID
    ) -> Dict:
        """
        ...
        try:
            with zipfile.ZipFile(io.BytesIO(file_data)) as zf:
                ...
                for member in zf.infolist():
                    ...
                    if (
                        curr == "skill.md"
                        or curr == target_readme
                        or curr.endswith("/skill.md")
                    ):
    ...
    def _apply_skill_filters(
        ...
        if allowed_department_ids is not None:
            rbac_filter = or_(
                ...
                Skill.departments.any(
                    SkillDepartment.department_id.in_(allowed_department_ids)
                ),
            )
        ...
        if department_id:
            stmt = stmt.where(
                Skill.departments.any(SkillDepartment.department_id == department_id)
            )
    ...
    @staticmethod
    async def get_skill(
        db: AsyncSession, slug: str, version_number: Optional[int] = None
    ) -> Skill:
        """
        ...
        try:
            ...
            if version_number and version_number != skill.current_version:
                v_stmt = select(SkillVersion).where(
            ...
            raise HTTPException(
                status_code=500, detail="Internal server error while fetching skill"
            )
    ...
    async def set_latest_version(
        db: AsyncSession, slug: str, version_number: int
    ) -> Skill:
        """
    ...
    async def delete_skill(db: AsyncSession, slug: str):
        ...
        try:
            ...
            pool = await get_arq_pool()
            await pool.enqueue_job(
                "delete_skill_task", str(skill.id), _queue_name="skills_queue"
            )
    ...
    async def update_skill(db: AsyncSession, slug: str, req_data: dict) -> Skill:
        ...
        is_department_explicit = "department_ids" in req_data.get(
            "_explicit_fields", []
        )
        ...
        if name is not None and name != skill.name:
            ...
            if res.scalars().first():
                raise HTTPException(
                    status_code=409, detail=f"Skill with name '{name}' already exists."
                )
            skill.name = name
        ...
        if (
            ...
            or is_scope_explicit
            or department_ids is not None
            or is_department_explicit
        ):
            # Combine all unique department IDs from both sources
            all_depts = list(
                set(
                    filter(
                        None, (department_ids or []) + ([scope_id] if scope_id else [])
                    )
                )
            )
            ...
            if all_depts:
                ...
                await db.execute(
                    sa.delete(SkillDepartment).where(
                        SkillDepartment.skill_id == skill.id
                    )
                )
                for d_id in all_depts:
        ...
        if skill_id and base_version:
            ...
            if v_obj and v_obj.storage_path:
                ...
                await asyncio.to_thread(
                    storage_service.copy_prefix,
                    v_obj.storage_path,
                    contribution.storage_path,
                )
            else:
                logger.warning(
                    f"Base version {base_version} for skill {skill_id} not found."
                )
        else:
    ...
    async def bulk_change_scope(
        ...
        await db.execute(
            sa.delete(SkillDepartment).where(SkillDepartment.skill_id.in_(skill_ids))
        )
        if dept_ids:
    ...
    async def materialize_approved_contribution(
        ...
        # A contribution without stored files cannot be hashed or copied; fail
        # fast instead of threading None into the storage layer.
        if contribution.storage_path is None:
            raise HTTPException(
                400, "This contribution has no stored files to materialize."
            )

        # 1. Calculate contribution hash and check for duplicates
        contribution_hash = storage_service.calculate_prefix_hash(
            contribution.storage_path
        )
        ...
        if contribution.skill_id:
            ...
            if v_dup_res.scalars().first():
                ...
                raise HTTPException(
                    400,
                    "This contribution is identical to an existing version of the skill.",
                )
        ...
        scope_ids = (
            final_scope_ids if final_scope_ids is not None else contribution.scope_ids
        )
        ...
        if scope_type == "department" and scope_ids:
            ...
            from cygnus.runtime.database.models import SkillDepartment

            # Remove old links (if updating)
            await db.execute(
                delete(SkillDepartment).where(SkillDepartment.skill_id == skill.id)
            )
            # Add new links
        ...
        await asyncio.to_thread(
            storage_service.copy_prefix, contribution.storage_path, v_path
        )
    ...
    async def create_contribution_from_zip(
        ...
        try:
            with zipfile.ZipFile(io.BytesIO(file_data)) as zf:
                ...
                has_single_root = len(first_segments) == 1 and all(
                    "/" in p for p in all_files
                )
                ...
                for member in zf.infolist():
                    ...
                    is_text = any(
                        member_path.lower().endswith(ext)
                        for ext in [
                            ".py",
                            ".md",
                            ".txt",
