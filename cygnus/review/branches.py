...
class BranchMergeConflict(Exception):
    ...
    def __init__(
        self, page_title: str, page_slug: str, current_version: int, base_version: int
    ):
...
async def submit_wiki_branch(
    db: AsyncSession, branch: WikiBranch, actor: Employee
) -> list[WikiPageDraft]:
    ...
    if branch.status != "draft":
        raise InvalidTransition(
            f"Cannot submit for merge while the branch is in '{branch.status}' state"
        )

    count_stmt = select(func.count(WikiPageDraft.id)).where(
        WikiPageDraft.branch_id == branch.id
    )
    count = (await db.execute(count_stmt)).scalar_one()
    if count == 0:
        raise InvalidTransition(
            "Your contribution branch is empty. Add a page draft before submitting."
        )
...
async def close_wiki_branch(
    ...
    *,
    ...
    if not is_author and actor.role != "admin" and not reviewer_override:
        raise InvalidTransition(
            "You do not have permission to close or cancel this branch"
        )
...
async def merge_wiki_branch(
    ...
    if branch.status != "pending_merge":
        raise InvalidTransition(
            "A branch can only be merged while it is in 'pending_merge' state"
        )
    ...
    for draft in drafts:
        ...
        if (
            ...
            raise BranchMergeConflict(
                page.title, page.slug, page.version, draft.base_version
            )
...
async def rebase_wiki_branch_draft(
    ...
    """Resolve one current branch conflict through the draft aggregate ledger."""
    ...
    if draft.author_id != author.id and author.role != "admin":
        raise InvalidTransition(
            "Only the original author may resolve this draft conflict"
        )
    if branch.status != "pending_merge":
        raise InvalidTransition(
            "A branch can only be rebased while it is pending merge"
        )
    if draft.status != "pending":
        raise InvalidTransition("Only a pending draft can be rebased")
    ...
    if draft.base_version is None or draft.base_version >= page.version:
        raise InvalidTransition(
            "Draft is already based on the current wiki page version"
        )
    ...
    try:
        _ = await record_draft_update(
            ...
            extra_payload={
                ...
                "page_id": str(page.id),
                "base_page_version": page.version,
            },
            lock=False,
        )
    ...
    await log_audit(
        ...
        "wiki_page_draft",
        str(draft.id),
        reason=f"draft_version={draft.version};base_page_version={page.version}",
    )
    return draft
