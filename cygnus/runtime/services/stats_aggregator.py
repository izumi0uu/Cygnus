...
Metric categories (see also docs in `cygnus/runtime/routers/admin_stats.py`):
...
def _norm_text(text: str) -> str:
    ...
    tokens = [
        ...
        if t and t not in _STOPWORDS_EN and t not in _STOPWORDS_VI
    ]
...
async def _rollup_content(
    ...
    rows.append(
        {"metric_key": "wiki.pages.created", "value_numeric": float(created_today)}
    )
    ...
    rows.append(
        {"metric_key": "wiki.pages.updated", "value_numeric": float(updated_today)}
    )
    ...
    revisions = (
        await session.execute(
            select(func.count(WikiPageRevision.id)).where(
                and_(
                    WikiPageRevision.created_at >= win_start,
                    WikiPageRevision.created_at < win_end,
                )
            )
        )
    ).scalar() or 0
    rows.append(
        {"metric_key": "wiki.revisions.daily", "value_numeric": float(revisions)}
    )

    # Pages by page_type (snapshot, dimensioned)
    type_rows = (
        await session.execute(
            select(WikiPage.page_type, func.count(WikiPage.id)).group_by(
                WikiPage.page_type
            )
        )
    ).all()
...
async def _rollup_contribution(
    session: AsyncSession, target_date: date
) -> list[dict[str, Any]]:
    win_start, win_end = _window(target_date)
    rows: list[dict[str, Any]] = []
    ...
    created = (
        await session.execute(
            select(func.count(WikiPageDraft.id)).where(
                and_(
                    WikiPageDraft.created_at >= win_start,
                    WikiPageDraft.created_at < win_end,
                )
            )
        )
    ).scalar() or 0
    ...
    ttr = (
        await session.execute(
            select(
                func.avg(
                    func.extract(
                        "epoch", WikiPageDraft.reviewed_at - WikiPageDraft.created_at
                    )
                )
            ).where(
                and_(
    ...
    rows.append(
        {
            "metric_key": "draft.time_to_review_avg_seconds",
            "value_numeric": float(ttr) if ttr is not None else None,
        }
    )
    ...
    rows.append(
        {
            ...
            "value_json": {
                "items": [
                    {
                        "reviewer_id": str(rid) if rid else None,
                        "name": name or "(unknown)",
                        "count": int(n),
                    }
    ...
    rows.append(
        {
            "metric_key": "compile_plan.pending_review",
            "value_numeric": float(plan_pending),
        }
    )
...
async def _rollup_usage(
    ...
    dau = (
        await session.execute(
            select(func.count(distinct(MCPQueryLog.employee_id))).where(
                and_(
                    MCPQueryLog.created_at >= win_start,
                    MCPQueryLog.created_at < win_end,
    ...
    latency = (
        await session.execute(
            select(func.avg(MCPQueryLog.latency_ms)).where(
                and_(
                    MCPQueryLog.created_at >= win_start,
                    MCPQueryLog.created_at < win_end,
    ...
    rows.append(
        {
            "metric_key": "mcp.latency_ms_avg",
            "value_numeric": float(latency) if latency is not None else None,
        }
    )
    ...
    rows.append(
        {
            ...
            "value_json": {
                "items": [{"tool_name": t, "count": int(n)} for t, n in by_tool]
            },
        }
    )
    ...
    rows.append(
        {
            ...
            "value_json": {
                "items": [
                    {
                        "employee_id": str(eid) if eid else None,
                        "name": name or "(unknown)",
                        "count": int(n),
                    }
...
async def _rollup_gaps(
    ...
    grouped: dict[str, dict[str, Any]] = defaultdict(
        lambda: {"count": 0, "samples": [], "requester_ids": set()}
    )
    ...
    rows.append(
        {
            ...
            "value_json": {
                "items": [
                    {"resource_type": rt, "count": int(n)} for rt, n in by_resource
                ]
            },
        }
    )
    ...
    async with async_session_factory() as session:
        for name, fn in [
            ...
            try:
                ...
                count = await _upsert_metrics(
                    session, target_date=target_date, rows=rows
                )
                written[name] = count
            except Exception as exc:  # noqa: BLE001
                logger.exception(
                    f"stats: {name} rollup failed for {target_date}: {exc}"
                )
                written[name] = -1
