...
- `GovernanceFeedbackRoute` 是唯一的 durable queue truth，在 `(feedback_signal_id, route_kind)` 上唯一，`route_kind` 为 `review|refresh`，`lifecycle_state:"queued"`。映射已冻结：`low_rating` → queued `review`，`stale_answer` → queued `refresh`，其他所有可接受 type → 无 route 的 `recorded_only`。
- Routed response 暴露 `route_id`、`route_ref=feedback-route:{uuid}`、`route_kind`、`route_state:"queued"` 与 `routing_state:"review_queued"|"refresh_queued"`，且恰有一个 queue flag 为真。Non-routed response 暴露 null route fields、`routing_state:"recorded_only"` 与两个都为假的 queue flags。不引入 feedback worker 或 consumer；queued 只表示 durable intent，绝不表示 review 或 refresh 工作已完成。
