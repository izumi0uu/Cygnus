...
- CYG-138 closes the durable delivery seam for governed propagation: publish stages one actor-bound outbound delivery receipt per target surface with the canonical approved publication/object/binding/source snapshot and its SHA-256 desired digest, expected page/approval/binding versions, idempotency key, bounded attempt budget, and correlation metadata. The worker sweep dispatches through configured internal-channel targets only (allowlisted, DNS-validated per redirect, HTTPS outside local/test, bounded retry + in-memory circuit + dead-letter exhaustion); retries re-send the exact frozen bytes so they cannot duplicate effect. Only a signed acknowledgment binding publication/channel/version/digest can set a propagation to `synced`; exact replay returns one receipt while drift conflicts. The fake consumer proves the delivery adapter contract only. A real signed customer-owned endpoint and a two-turn client session remain external Production V1 blockers, not production evidence.
