...
def test_governance_router_surface_adapters_exist() -> None:
    ...
    for relative_path in expected:
        assert Path(relative_path).is_file(), (
            f"missing governance adapter file: {relative_path}"
        )
...
def test_governance_router_is_thin_assembly() -> None:
    ...
    assert (
        "from cygnus.runtime.routers.governance.audit import router as governance_audit_router"
        in text
    )
    assert (
    ...
    assert (
        "from cygnus.runtime.routers.governance.session_bridge import router as session_bridge_router"
        in text
    )
