...
// Drift watch = the drift risks as freshness status cards; coverage proxied by distinct objects watched.
export default function CoverageDrift() {
  ...
  const driftObjs = new Set(rows.map((it) => it.object_ref)).size
  ...
  return (
    <>
        ...
        {rows.map((it) => (
          <div key={it.risk_id} className="overflow-hidden rounded-xl border border-border bg-card shadow-soft">
            <div
              ...
              style={{ background: 'color-mix(in srgb, var(--high) 8%, transparent)' }}
            >
              <span className="h-2.5 w-2.5 rounded-full" style={{ background: 'var(--high)' }} />
              <span className="text-[13px] font-bold" style={{ color: 'var(--high)' }}>{t('drift.flag')}</span>
      ...
      {rows.length === 0 ? (
        <div className="rounded-xl border border-border bg-card px-[18px] py-10 text-center text-sm text-muted-foreground">{t('drift.empty')}</div>
      ) : ok > 0 ? (
