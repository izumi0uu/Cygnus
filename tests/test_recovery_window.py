...
class RecoveryWindowTests(unittest.TestCase):
    def test_build_recovery_window_returns_before_after_surface(self) -> None:
        ...
        alignment_view = cast(dict[str, object], surface["before_after_alignment_view"])
        closure_judge = cast(dict[str, object], surface["closure_judge"])

        self.assertEqual(surface["surface_id"], "recovery-window")
        self.assertEqual(surface["assessment"], "false_recovery")
        self.assertTrue(cast(dict[str, object], surface["rewrite_delta"])["improved"])
        self.assertEqual(cast(dict[str, object], surface["rewrite_delta"])["delta"], -6)
        self.assertIn(
            "audience_truth",
            cast(list[str], alignment_view["residual_truth_planes"]),
        )
        self.assertEqual(
            closure_judge["recommendation"],
            "continue_with_lightweight_follow_up",
        )
        self.assertIn(
            "open_audience_mismatch_review",
            cast(list[str], surface["continue_commands"]),
        )
        self.assertIn(
            "downstream_reality_check:cmd-publish-1",
            cast(list[str], surface["supporting_links"]),
        )
    ...
    def test_query_returns_surface_for_known_command(self) -> None:
        ...
        self.assertEqual(
            cast(dict[str, object], payload["publish_conflict_delta"])["after_value"],
            1,
        )
