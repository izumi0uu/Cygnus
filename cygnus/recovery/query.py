...
class GovernanceOverviewQuery:
    ...
    def __post_init__(self) -> None:
        normalized = tuple(
            command_id.strip() for command_id in self.command_ids if command_id.strip()
        )
        if not normalized:
...
def get_downstream_reality_check_surface(
    ...
    *,
    ...
    refs = (
        ...
        else (sample_reality_check_command_ref(),)
    )
    feed = (
        tuple(feedback_feed)
        if feedback_feed is not None
        else sample_reality_check_feedback()
    )
    ...
    raise ValueError(
        f"downstream reality check not found for command_id={query.command_id}"
    )
...
def get_recovery_window_surface(
    ...
    *,
    ...
    refs = (
        tuple(command_refs)
        if command_refs is not None
        else sample_recovery_command_refs()
    )
    before = (
...
def get_governance_overview_surface(
    ...
    *,
    ...
    filtered_windows = tuple(
        ...
        if window.command_ref.command_id in query.command_ids
    )
    if not filtered_windows:
        raise ValueError(
            "governance overview requires at least one matching recovery window"
        )
    ...
    return build_governance_overview(
        ...
        residual_risks=tuple(
            risk for risk in residuals if risk.command_id in query.command_ids
        ),
    )
