...
in PostgreSQL) instead of producing chunk embeddings. See cygnus/backend/ai/wiki_compiler.py.

Start with:
    arq cygnus.backend.worker.WorkerSettings
"""
...
from cygnus.backend.utils.progress import ProgressTracker  # noqa: E402
...
async def finalize_verbatim_source(session, source, tracker) -> dict:
    ...
    straight to ready.
    """
    from cygnus.backend.services.verbatim_service import index_verbatim_source
...
async def ingest_file_task(ctx: dict, source_id: str):
    ...
    from cygnus.backend.database.models import Source, SourceImage
    from cygnus.backend.services.image_service import extract_images
    from cygnus.backend.services.kb_service import (
        _extract_text_from_file,
        _inline_image_markers,
    )
    from cygnus.backend.services.source_outline import assemble_full_text, build_outline
    from cygnus.backend.services.storage_service import storage_service
    from cygnus.backend.utils.tokens import count_tokens
    ...
    async with async_session_factory() as session:
        ...
        try:
            ...
            async def _mark_error_file() -> None:
                from cygnus.backend.database import async_session_factory as _sf
                from cygnus.backend.database.models import Source as _Source
...
async def ingest_url_task(ctx: dict, source_id: str):
    ...
    from cygnus.backend.database import async_session_factory
    from cygnus.backend.database.models import Source
    from cygnus.backend.services.kb_service import _extract_text_from_url
    from cygnus.backend.services.source_outline import assemble_full_text, build_outline
    from cygnus.backend.utils.tokens import count_tokens
...
async def ingest_skill_task(ctx: dict, skill_id: str, version_id: str, file_path: str, file_name: str):
    ...
    from cygnus.backend.database import async_session_factory
    from cygnus.backend.database.models import Skill, SkillVersion
    from cygnus.backend.services.storage_service import storage_service
    ...
    async with async_session_factory() as session:
        ...
        try:
            ...
            from cygnus.backend.services.kb_service import _guess_content_type
...
async def delete_skill_task(ctx: dict, skill_id: str):
    ...
    """
    from cygnus.backend.database import async_session_factory
    from cygnus.backend.database.models import Skill
    from cygnus.backend.services.storage_service import storage_service
...
async def reembed_all_pages_task(ctx: dict, job_id: str) -> None:
    ...
    from cygnus.backend.ai.registry import ProviderRegistry
    from cygnus.backend.database import async_session_factory
    from cygnus.backend.database.models import EmbeddingJob, Source, WikiPage
    from cygnus.backend.services.config_service import (
        ACTIVE_EMBEDDING_MODEL_KEY,
        ConfigService,
    )
    from cygnus.backend.services.embedding_storage import (
...
async def ingest_map_reduce_task(ctx: dict, source_id: str):
    ...
    """
    from cygnus.backend.ai.mrp.pipeline import run_mrp_pipeline
    from cygnus.backend.ai.registry import ProviderRegistry
    from cygnus.backend.database import async_session_factory
    from cygnus.backend.database.models import KnowledgeType, Source
...
async def ingest_refine_task(ctx: dict, source_id: str):
    ...
    """
    from cygnus.backend.ai.mrp.pipeline import run_refine_pipeline
    from cygnus.backend.ai.registry import ProviderRegistry
    from cygnus.backend.database import async_session_factory
    from cygnus.backend.database.models import KnowledgeType, Source
...
async def regenerate_plan_task(ctx: dict, source_id: str, user_note: str):
    ...
    """
    from cygnus.backend.ai.mrp.reducer import reconcile_with_kb, run_planning_call
    from cygnus.backend.ai.registry import ProviderRegistry
    from cygnus.backend.database import async_session_factory
...
async def sweep_stuck_ai_review_cron(ctx: dict):
    ...
    from cygnus.backend.database import async_session_factory
    from cygnus.backend.database.models import WikiPageDraft
...
async def daily_stats_rollup_cron(ctx: dict):
    ...
    from cygnus.backend.services.stats_aggregator import run_daily_rollup
...
async def ai_pre_review_draft_task(
    ...
    """
    from cygnus.backend.services.ai_review import run_async_checks
    _ = ctx
